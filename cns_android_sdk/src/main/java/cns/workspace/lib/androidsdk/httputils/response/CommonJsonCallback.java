...
public class CommonJsonCallback extends CommonBase implements Callback {
                ...
                if(ioexception != null){
                    if (ioexception.getMessage() != null){
                        if (ioexception.getMessage().equals("Canceled")){
                            mListener.onFailure(new OkHttpException(OkHttpException.CANCEL_REQUEST, CANCEL_MSG,ioexception));
                        }else {
                            mListener.onFailure(new OkHttpException(OkHttpException.NETWORK_ERROR, NET_MSG,ioexception));
                        }
                    }
                }
            }
    ...
    private void handleResponse(Object responseObj) {
        ...
        try {
            ...
            e.printStackTrace();
            mListener.onFailure(new OkHttpException(OkHttpException.JSON_ERROR, JSON_MSG , JSON_MSG_TYPEREFERENCE));//json解析错误
        }
    }
}